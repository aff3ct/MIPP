...
#of the correct lmul with something a vmv intrinsic 
    ...
    for f in reductions:
        ...
        if f in implems_dict:
            for ff in implems_dict[f]:
                ir = ff["template"]["code"]
                #get only line 1 of the code template 
                line1_ir = ir.splitlines()[1]
                line2_ir = ir.splitlines()[2]
                #line3_ir = ir.splitlines()[3]
                #resolve this line with lmul = 1
                for dt in ff["datatypes"]:
                    ...
                    #line 4 -> return r0. This line is not changed.
...
def gen_c_functions(isa, file, funcs, implems,lmul=0, reductions_fix=False):
	#print(lmul)
	for f in implems:
		if f in funcs:
			for ff in implems[f]:
				for dt in ff["datatypes"]:
					...
					if False:
						...
						#print(build_proto(funcs[f]["proto"], dt_par, dt_ret, isa, func_name,lmul=lmul),lmul)
...
def gen_c_missing_functions(isa, file, funcs,lmul=0):
	for f in funcs:
...
def gen_c_defines_rvv_ls(file, isa_name, rvv_size):
    print("#define MIPP_" + isa_name.upper() + "1_0" + "_RVD_SIZE_BIT " + str(rvv_size), file=file)
    print("#define MIPP_" + isa_name.upper() + "1_0" + "_RVD_SIZE_BYTE " + str(rvv_size + "/ 8"), file=file)
...
def gen_mipp_rvv(vlen="__riscv_v_fixed_vlen"):
...
typedef double float64_t;//remove after debug"""
    ...
    print(j2_template.render(vlen=vlen), file=file)
    ...
    for lmul in [0] + all_lmul:
        ...
        gen_c_functions(resolved_isa, file, copy_mipp_funcs, implems_rvv, lmul=lmul)
        ret = fix_reductions(resolved_isa, isa_rvv_lmul1, copy_mipp_funcs, implems_emu_rvv, lmul)

        gen_c_functions(resolved_isa, file, copy_mipp_funcs, implems_emu_rvv, lmul=lmul, reductions_fix=ret)
        gen_c_missing_functions(resolved_isa, file, copy_mipp_funcs, lmul=lmul)
