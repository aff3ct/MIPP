...
# Note: your original obj uses cpp decl shapes (vectorSize uses mipp::N<T>())
tpl_decl = {
...
tpl_load = {
    "1arg": """\t//{{reg_type}} r1(inputs);//waiting for load to be fixed in obj layer
\t{{reg_type}} r1; r1.r = mipp::load<T>(inputs1);""",
    "2args": """\t//{{reg_type}} r1(inputs1), r2(inputs2);//waiting for load to be fixed in obj layer
\t{{reg_type}} r1, r2; r1.r = mipp::load<T>(inputs1); r2.r = mipp::load<T>(inputs2);""",

    "1arg_msk": """\t{{msk_type}} m1; m1.m = mipp::set_k<{{dt_ext}}>(inputs1); //{{reg_type}} r1; r1.r = mipp::toreg(m1.m);""",
    "2args_msk": """\t{{msk_type}} m1, m2; m1.m = mipp::set_k<T>(inputs1); m2.m = mipp::set_k<T>(inputs2);""",

    "1arg_set": """\t{{reg_type}} r1; r1.r = mipp::{{func}}<{{dt_ext}}>(inputs1);""",
    "1arg_set1k": """\t{{msk_type}} m1; m1.m = mipp::set1_k<{{dt_ext}}>(inputs1); {{reg_type}} r1; r1.r = mipp::toreg(m1.m);""",
}

tpl_op = {
    "operator_1arg": """\tr1.r = mipp::{{func}}(r1.r);""",
    "operator_2args": """\t{{reg_type}} r3 = r1 {{op}} r2;""",
    "fn_call_2args": """\t{{reg_type}} r3; r3.r = mipp::{{func}}(r1.r, r2.r);""",

    "store": """\tmipp::store(inputs2, r1.r);""",

    "msk_operator_1arg": """\t{{msk_type}} m2; m2.m = mipp::{{func}}(m1.m); {{reg_type}} r1; r1.r = mipp::toreg(m2.m);""",
    "msk_operator_2args": """\t{{msk_type}} m3; m3.m = mipp::{{func}}(m1.m, m2.m); {{reg_type}} r1; r1.r = mipp::toreg(m3.m);""",
}
...
tpl_assert = {
    "operator_2args": """\t\tREQUIRE(r3[i] == res);""",
    "operator_1arg": """\t\tREQUIRE(r1[i] == res);""",
    "store": """\t\tREQUIRE(inputs2[i] == res);""",

    "mask_2args": """\t\tif(res) REQUIRE(r1[i] != 0); else REQUIRE(r1[i] == 0);""",
}
...
def mk_prototype_registry() -> dict:
    ...
    # andnb without mask: ~inputs1[i] & inputs2[i]
    andnb = override(arith_2args, **{
        K_LOOP_BODY: "\t\tT res = (~inputs1[i]) &  inputs2[i];",
        K_OP: tpl_op["fn_call_2args"],  # matches your original: use mipp::andnb(...) call
    })
    ...
    blend = lang_proto(
        ...
        operation="\t{{reg_type}} r3; r3.r = mipp::blend(r1.r, r2.r, m1.m);",
