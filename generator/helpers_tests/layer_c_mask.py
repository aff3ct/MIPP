...
# FN decl
# --------------------------------------------
FUNC_DECL = """static void test_cmipp_{{func}}_{{dt_ext}}{{mask_kind}}{{lmul_suffix}}(){"""

FUNC_DECL_GATHER = """static void test_cmipp_{{func}}_{{dt_ext}}_{{dt_ext}}{{mask_kind}}{{lmul_suffix}}(){"""
FUNC_DECL_SCATTER = """static void test_cmipp_{{func}}_{{dt_ext}}_{{dt_ext}}{{mask_kind}}{{lmul_suffix}}(){"""
...
LOAD_GATHER = """
rvd_uint{{type_size}}{{lmul_suffix}}_t ri1 = mipp_load_uint{{type_size}}{{lmul_suffix}}(indexes);
rvd_scalar_uint{{type_size}}{{lmul_suffix}}_t ris1 = mipp_scalar_load_uint{{type_size}}{{lmul_suffix}}(indexes);
"""

LOAD_SCATTER = """
{{reg_type}} r1 = mipp_load_{{dt_ext}}{{lmul_suffix}}(inputs1);
rvd_uint{{type_size}}{{lmul_suffix}}_t ri1 = mipp_load_uint{{type_size}}{{lmul_suffix}}(indexes);

{{reg_type_scalar}} rs1 = mipp_scalar_load_{{dt_ext}}{{lmul_suffix}}(inputs1);
rvd_scalar_uint{{type_size}}{{lmul_suffix}}_t ris1 = mipp_scalar_load_uint{{type_size}}{{lmul_suffix}}(indexes);
"""
...
OP_REG_VAL = """\t{{reg_type}} r3 = mipp_{{func}}_{{dt_ext}}{{mask_kind}}{{lmul_suffix}}({{mask_args}}r1, input1);\n\t{{reg_type_scalar}} s3 = mipp_scalar_{{func}}_{{dt_ext}}{{mask_kind}}{{lmul_suffix}}({{mask_args_scalar}}s1, input1);"""
...
AS_GATHER = """\t\tREQUIRE(mipp_get_{{dt_ext}}{{lmul_suffix}}(r2, i) == mipp_scalar_get_{{dt_ext}}{{lmul_suffix}}(s2, i));"""
...
+ """\n\t\t}""",
    ...
    # always divide by 2 so no 0 check
    "div2" : {
        "loop_assert" :"""\t\t {{dt_ext}}_t res1 = mipp_get_{{dt_ext}}{{lmul_suffix}}(r3, i);\n \t\t{{dt_ext}}_t res2 = mipp_scalar_get_{{dt_ext}}{{lmul_suffix}}(s3, i);\n"""
+ "{% if is_int%}"
+ "\t\tREQUIRE(abs_diff::abs_diff(res1,res2) == 0);"
+ "{% else %}"
+ "\n\t\t{{dt_ext}}_t tol  = 1e-5f * abs_diff::abs_diff(res2) + 1.0f;"
