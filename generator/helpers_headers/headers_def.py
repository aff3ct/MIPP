...
import tools  # for dynamic attribute resolution
...
def _load_funcs_registry():
    ...
    json_path = os.path.join(current_dir, "funcs_registry.json")
    with open(json_path, "r") as f:
        data = json.load(f)

    # DATATYPES_MAP containing all possible lists and elements
    DATATYPES_MAP = {
        "all_float": all_float,
        "all_uint": all_uint,
        "all_int": all_int,
    ...
    # Resolve any list from tools if it is not in the map above
    # e.g., all_datatypes_idx_pair, all_datatypes_same_size
    def get_datatypes_from_tools(name):
        if hasattr(tools, name):
            return getattr(tools, name)
        raise ValueError(f"Unknown datatypes set: {name}")
    ...
    def resolve_datatypes(spec):
        ...
        for s in spec:
            if s in DATATYPES_MAP:
                ...
                resolved.extend(get_datatypes_from_tools(s))
        return resolved
    ...
    # Resolve mipp_funcs
    mipp_funcs = {}
    for k, v in data["mipp_funcs"].items():
        mipp_funcs[k] = {
    ...
    implems_scalar = {}
    for k, v in data["implems_scalar"].items():
        implems_scalar[k] = []
        for item in v:
    ...
    return protos, data["mipp_funcs_concepts"], mipp_funcs, implems_scalar

protos, mipp_funcs_concepts, mipp_funcs, implems_scalar = _load_funcs_registry()
