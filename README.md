...
| `gather`        | `Reg  <TD,TI> gather    (const TD* mem, const Reg<TI> idx)`                            | Gathers elements from `mem` to a register. Select elements according to the indices in `idx`.                                                                      | `double`, `float`, `int64_t`, `int32_t`, `int16_t`, `int8_t` |
...
| `maskzgat`      | `Reg  <TD,TI> gather    (const Msk<N> m, const TD* mem, const Reg<TI> idx)`            | Gathers elements from `mem` to a register (according to the mask `m`). Select elements according to the indices in `idx` (puts zero when the mask value is false). | `double`, `float`, `int64_t`, `int32_t`, `int16_t`, `int8_t` |
